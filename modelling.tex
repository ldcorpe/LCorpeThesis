...
\ifNewAnalysis
The parametrisations of the \mgg distributions for the \ggH process (with $\mH=125\GeV$) in two inclusive categories are shown in \Fig~\ref{fig:model:functionalform} for both the \DCBpG and sum of Gaussians, where the \RV and \WV components have been summed according to their relative event count. %The agreement between the distribution and the parametrisation when the simpler paramertisation can be seen from the $\chi^2/\text{NDOF}$values.
\else
